...
//
	...
	@PostMapping("/printGeneralInvoicepdf/{cid}/{bid}/{invoiceNo}/{assesmentId}/{igmTransId}/{selectedInvoice}")
	public ResponseEntity<?> printGeneralInvoicepdf(@PathVariable("cid") String cid, @PathVariable("bid") String bid ,@PathVariable("invoiceNo") String invoiceNo,
			@PathVariable("assesmentId") String assesmentId,@PathVariable("igmTransId") String igmTransId,
			@PathVariable("selectedInvoice") String selectedInvoice) throws DocumentException{
...
//			List<Map<String,String>> TotalCal = new ArrayList<>();
			for(String sac :bysacTotal.keySet()) {
				...
				entry.put("sact1",sact1);
				boolean isIgst = "Y".equals(invdtl.get("Igst"));
				Key = isIgst;

				for (Map<String, Double> taxpair : taxRatePairs) {
				    BigDecimal totalAmt = bysacTotal.get(sac);

				    BigDecimal cgstRate = BigDecimal.valueOf(taxpair.get("CGST"));
				    BigDecimal sgstRate = BigDecimal.valueOf(taxpair.get("SGST"));
				    BigDecimal totalTaxPercent = cgstRate.add(sgstRate).divide(BigDecimal.valueOf(100));

				    BigDecimal taxAmount, finalAmount;

				    if (isIgst) {
				        taxAmount = totalAmt.multiply(totalTaxPercent);
				        finalAmount = totalAmt.add(taxAmount);
				    } else {
				        BigDecimal halfTax = totalTaxPercent.divide(BigDecimal.valueOf(2), 2, RoundingMode.HALF_UP);
				        taxAmount = totalAmt.multiply(halfTax);
				        finalAmount = totalAmt.add(taxAmount.multiply(BigDecimal.valueOf(2)));
				    }

				    String taxAmountStr = String.format("%.2f", taxAmount);
				    String finalAmountStr = String.format("%.2f", finalAmount);
				    String taxPercStr = String.format("%.2f", totalTaxPercent.multiply(BigDecimal.valueOf(100)));

				    boolean isMatch = importinvoiceservice.sacNoAndTaxpercMatch(sac, taxPercStr.trim(), itemList);

				    if (isMatch) {
				        entry.put("taxAmountI1", taxAmountStr);
				        entry.put("finalamtI1", finalAmountStr);
				    } else {
				        entry.put("taxAmountI1Alt", null);
				    }
				}
...
//			System.out.println("total Cal "+TotalCal.toString());
		...
		String invType = "noc".equals(selectedInvoice) ? "GENERAL RECEIVING INVOICE" : "GENERAL DELIVERY INVOICE";
		context.setVariable("invType", invType);
		
		String htmlContent = templateEngine.process("GeneralInvoicePrint.html", context);
//
		ITextRenderer renderer = new ITextRenderer();
		renderer.setDocumentFromString(htmlContent);
		renderer.layout();
