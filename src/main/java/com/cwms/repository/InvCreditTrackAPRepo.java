...
public interface InvCreditTrackAPRepo extends JpaRepository<InvCreditTrackAP, String> {
	...
	@Query(value="select i.invoiceNo,i.partyId,p1.partyName,c.accSrNo,c.creditType,c.billingParty,c.comments,c.importerId,c.impSrNo,"
			...
			+ "from InvCreditTrackAP i "
			+ "Left Outer Join Party p1 ON i.companyId=p1.companyId and i.branchId=p1.branchId and i.partyId=p1.partyId "
			+ "Left Outer Join Cfinvsrvap c ON i.companyId=c.companyId and i.branchId=c.branchId and i.invoiceNo=c.invoiceNo and "
			+ "i.partyId=c.partyId "
	...
	@Query(value="select i.invoiceNo,i.partyId,p1.partyName,c.accSrNo,c.creditType,c.billingParty,c.comments,c.importerId,c.impSrNo,"
			...
			+ "Left Outer Join Party p2 ON c.companyId=p2.companyId and c.branchId=p2.branchId and c.importerId=p2.partyId "
			+ "Left Outer Join Party p3 ON i.companyId=p3.companyId and i.branchId=p3.branchId and i.tdsDeductee=p3.partyId "
			+ "where i.companyId=:cid and i.branchId=:bid and i.status='A' and c.status='A' and i.partyId=:id order by i.transDate asc")
	List<Object[]> getBeforeSearchDataForeceipt1(@Param("cid") String cid,@Param("bid") String bid,@Param("id") String id);
	
	
	@Query(value="select COALESCE(c.creditAdjustAmount,0) from InvCreditTrackAP c where c.companyId=:cid and c.branchId=:bid and c.invoiceNo=:invNo and c.status = 'A'")
	BigDecimal getCreditAdjAmtByInvoiceNo(@Param("cid") String cid,@Param("bid") String bid,@Param("invNo") String invNo);
	
	@Modifying
	@Transactional
	@Query(value = "UPDATE InvCreditTrackAP c " +
