...
public interface CFSTarrifServiceRepositoryQuatation extends JpaRepository<CFSTariffServiceQuatation, String> {
	
	
	@Query(value = "SELECT s " + 
            "FROM CFSTariffServiceQuatation s " +
            "WHERE s.companyId = :companyId " + 
            "AND s.branchId = :branchId " + 
            "AND s.status = 'A' " + 
            "AND s.cfsTariffNo = :tariffNo " + 
            "AND s.cfsAmendNo = :cfsAmendNo")
List<CFSTariffServiceQuatation> getForTariffToTransfer(
 @Param("companyId") String companyId, 
 @Param("branchId") String branchId,
 @Param("tariffNo") String tariffNo, 
 @Param("cfsAmendNo") String cfsAmendNo
);
	...
	@Modifying
	@Transactional
	@Query("UPDATE CFSTariffServiceQuatation e " + "SET e.ammendStatus = 'A', " + "    e.editedBy = :user, "
	...
	@Modifying
	@Transactional
	@Query("UPDATE CFSTariffServiceQuatation e " + "SET e.containerSize = :containerSize, "
	...
	@Query("SELECT COUNT(e) > 0 FROM CFSTariffServiceQuatation e " + "WHERE e.companyId = :companyId AND e.branchId = :branchId "
	...
	@Query(value = "select DISTINCT NEW com.cwms.entities.CFSTariffServiceQuatation(s.serviceId,s.rate) from CFSTariffServiceQuatation s where "
			+ "s.companyId=:cid and s.branchId=:bid and s.status = 'A' and s.cfsTariffNo='CFS1000001' and s.rangeType='NA'")
	List<CFSTariffServiceQuatation> getGeneralTarrifData(@Param("cid") String cid, @Param("bid") String bid);

	@Query(value = "SELECT new com.cwms.entities.CFSTariffServiceQuatation("
			...
			+ "s.approvedDate, s.currencyId, s.ammendStatus, s.defaultChk, j.jarDtlDesc) " + "FROM CFSTariffServiceQuatation s "
	...
	List<CFSTariffServiceQuatation> getCFSTariffServiceByServiceId(@Param("companyId") String companyId,
	...
	@Query(value = "SELECT new com.cwms.entities.CFSTariffServiceQuatation("
			...
			+ "s.fromRange, s.toRange, s.rate, se.serviceShortDesc, s.currencyId) " + "FROM CFSTariffServiceQuatation s "
			+ "LEFT JOIN Services se "
			+ "ON s.companyId = se.companyId  AND s.branchId = se.branchId AND se.status <> 'D' AND s.serviceId = se.serviceId "
			+ "LEFT JOIN JarDetail j "
	...
	List<CFSTariffServiceQuatation> getForTariffReport(@Param("companyId") String companyId, @Param("branchId") String branchId,
			@Param("tariffNo") String tariffNo, @Param("cfsAmendNo") String cfsAmendNo);
	
	
	@Query(value = "SELECT new com.cwms.entities.CFSTariffServiceQuatation("
			+ "s.srNo, s.containerSize, j.jarDtlDesc, s.cargoType, s.rangeType, "
			+ "s.fromRange, s.toRange, s.rate, se.serviceShortDesc, s.currencyId, s.cfsAmendNo, s.serviceId, s.editedBy, s.editedDate) " + "FROM CFSTariffServiceQuatation s "
			+ "LEFT JOIN Services se "
			+ "ON s.companyId = se.companyId  AND s.branchId = se.branchId AND se.status <> 'D' AND s.serviceId = se.serviceId "
			+ "LEFT JOIN JarDetail j "
			+ "ON s.companyId = j.companyId AND j.status <> 'D' AND j.jarId = 'J00006' AND s.commodityCode = j.jarDtlId "
	...
	List<CFSTariffServiceQuatation> getForTariffAuditTrailReport(@Param("companyId") String companyId, @Param("branchId") String branchId,
			@Param("tariffNo") String tariffNo);
	...
	@Query(value = "SELECT c.serviceId, c.serviceUnit, c.rate, c.cfsTariffNo, c.cfsAmendNo, c.currencyId, c.srNo, "
	        ...
	        + "FROM CFSTariffServiceQuatation c "
