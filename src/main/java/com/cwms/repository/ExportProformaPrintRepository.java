...
public interface ExportProformaPrintRepository extends JpaRepository<ExportStuffTally, String>{
	...
	@Query(value ="select distinct a.container_no,DATE_FORMAT(a.approved_date,'%d %b %Y %T'),a.party_id,b.party_name BillingParty,b.pan_no,ba.address_1,ba.address_2,ba.address_3,ba.pin,h.jar_dtl_desc State,j.jar_dtl_desc Statecode,ba.gst_no,a.lock_down,u.user_name,v.user_name,a.bill_amt, "
			...
			+ "DATE_FORMAT( s.sb_date, '%d %b %Y' ) , DATE_FORMAT(s.min_carting_trans_date, '%d %b %Y' ), ROUND(s.sbweight, 2 ), '0.00' , s.area_used , s.commodity_description, s.transaction_type, a.invoice_type ,b.FINANCE_MAIL,a.Created_By,DATE_FORMAT(a.Invoice_Date,'%d/%m/%Y') ,s.is_bos "
			+ "from cfinvsrvpro  a left outer join cfassesmentsheetpro s on a.company_id = s.company_id and a.branch_id = s.branch_id and a.invoice_no = s.invoice_no and a.container_no = s.assesment_id and a.Profitcentre_Id = s.Profitcentre_Id  "
...
List<Object[]> getContainerAndSbDetails(@Param("companyId") String companyId, 
	...
	@Query(value=" select a.service_id,c.service_long_desc,count(a.service_id),sum(a.execution_unit),a.service_unit,sum(a.local_amt),IFNULL(ROUND( ((sum(b.invoice_Amt)/count(a.service_id))), 3 ), 0) UNITAMT,max(a.rate),DATE_FORMAT(a.approved_date,'%d %b %Y %T'),c.range_type,sum(a.execution_unit1),"
			+ "a.service_unit1,c.sac_code,IFNULL(j.Jar_Dtl_Desc,'N/A' ),IFNULL(ROUND( a.Tax_Perc_N, 1 ), 0 ) from cfinvsrvanxpro a  left outer join cfinvsrvdtlpro b on a.invoice_no=b.invoice_no and a.service_id=b.service_id  and a.company_Id=b.Company_Id and a.branch_Id=b.branch_Id  "
	...
	@Query(value ="select distinct a.SB_No , DATE_FORMAT( a.SB_Date, '%d %b %Y' ) , DATE_FORMAT(a.Min_Carting_Trans_Date, '%d %b %Y' ), a.Gross_Weight ,a.Invoice_Amt Amount , ROUND(SUM(f.Area_Occupied),2) , a.Commodity_Description,IFNULL(DATE_FORMAT(a.invoice_Cal_Date,'%d %b %Y' ),''),a.Exporter_Name "
			+ "from cfassesmentsheetpro a left outer join cfbacktotown e on e.company_id=a.company_id  and e.branch_id=a.branch_id  and a.Invoice_No=e.Invoice_No and e.Assesment_Id = a.Assesment_Id and e.Profitcentre_Id = a.Profitcentre_Id and a.sb_no = e.SB_No "
	...
	@Query(value = "select distinct a.container_no, a.container_size, a.container_type, '' , '' , DATE_FORMAT(a.Gate_In_Date,'%d-%m-%Y'), DATE_FORMAT(a.Stuff_Tally_Date,'%d-%m-%Y'), '', e.SB_No , DATE_FORMAT( e.SB_Date, '%d %b %Y' ) , DATE_FORMAT(a.Min_Carting_Trans_Date, '%d %b %Y' ), ROUND(g.Gross_Weight, 2 ), ROUND(g.FOB,2 ) Amount , ROUND(SUM(f.Area_Occupied),2) , e.Commodity,IFNULL(DATE_FORMAT(a.invoice_Cal_Date,'%d %b %Y' ),''),a.Exporter_Name  "
			+ "from cfassesmentsheetpro a left outer join cfbacktotown e on e.company_id=a.company_id  and e.branch_id=a.branch_id and a.Invoice_No=e.Invoice_No and e.Assesment_Id = a.Assesment_Id and e.Profitcentre_Id = a.Profitcentre_Id and a.sb_no = e.SB_No left outer join cfcrtg f on e.Company_Id=f.Company_Id  and e.Branch_Id=f.Branch_Id and e.Profitcentre_Id = f.Profitcentre_Id and e.SB_Trans_Id = f.SB_Trans_Id and e.SB_No = f.SB_No and e.SB_Line_No = f.SB_Line_No "
