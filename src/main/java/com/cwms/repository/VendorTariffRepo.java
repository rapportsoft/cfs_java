...
public interface VendorTariffRepo extends JpaRepository<VendorTariff, String> {

	@Query("SELECT new com.cwms.entities.VendorTariff( "
			...
			+ "WHERE c.companyId = :companyId " + "AND c.branchId = :branchId " + "AND c.cfsTariffNo = :cfsTariffNo "
			+ "AND c.cfsAmndNo = :cfsAmndNo " + "AND c.status = 'A'")
	VendorTariff getSavedTariffAuditTrailReport(String companyId, String branchId, String cfsTariffNo,
			String cfsAmndNo);

	@Query("SELECT NEW com.cwms.entities.VendorTariff(i.partyId, i.status, i.contractName, i.shippingLine,i.cha, i.importerId, i.exporterId, i.shippingAgent, i.forwarderId,i.consolerId, i.cfsTariffDate, i.cfsFromDate, i.cfsValidateDate) "
			+ "FROM VendorTariff i " + "WHERE " + "i.companyId = :companyId "
			+ "AND i.cfsTariffNo = :cfsTariffNo AND i.cfsAmndNo = :cfsAmndNo " + "AND i.branchId = :branchId "
			+ "AND i.ammendStatus = 'N' " + "AND i.status = 'A' ")
	VendorTariff getSavedTariffSubmit(@Param("companyId") String companyId, @Param("branchId") String branchId,
			String cfsTariffNo, String cfsAmndNo);
	...
	@Query("SELECT new com.cwms.entities.VendorTariff( "
			...
			+ "line.partyName, ch.partyName, imp.partyName, fo.partyName) " + "FROM VendorTariff c "
			+ "LEFT JOIN Party agent ON c.companyId = agent.companyId AND c.branchId = agent.branchId AND c.shippingAgent = agent.partyId AND agent.status != 'D' AND c.shippingAgent IS NOT NULL AND c.shippingAgent <> '' "
			+ "LEFT JOIN Party line ON c.companyId = line.companyId AND c.branchId = line.branchId AND c.shippingLine = line.partyId AND line.status != 'D' AND c.shippingLine IS NOT NULL AND c.shippingLine <> '' "
			+ "LEFT JOIN Party pa ON c.companyId = pa.companyId AND c.branchId = pa.branchId AND c.partyId = pa.partyId AND pa.status != 'D' AND c.partyId IS NOT NULL AND c.partyId <> '' "
			+ "LEFT JOIN Party fo ON c.companyId = fo.companyId AND c.branchId = fo.branchId AND c.forwarderId = fo.partyId AND fo.status != 'D' AND c.forwarderId IS NOT NULL AND c.forwarderId <> '' "
			...
			+ "WHERE c.companyId = :companyId " + "AND c.branchId = :branchId " + "AND c.cfsTariffNo = :cfsTariffNo "
			+ "AND c.cfsAmndNo = :cfsAmndNo " + "AND c.status = 'A'")
	VendorTariff getSavedTariff(String companyId, String branchId, String cfsTariffNo, String cfsAmndNo);

	@Query("SELECT NEW com.cwms.entities.VendorTariff(i.cfsTariffNo, i.cfsAmndNo, i.status, i.ammendStatus) "
			+ "FROM VendorTariff i " + "WHERE " + "i.companyId = :companyId "
			+ "AND i.cfsTariffNo = :cfsTariffNo AND i.cfsAmndNo = :cfsAmndNo " + "AND i.branchId = :branchId "
			+ "AND i.status = 'A' ")
	VendorTariff getSavedTariffValidateUpdatable(@Param("companyId") String companyId, @Param("branchId") String branchId,
			String cfsTariffNo, String cfsAmndNo);
	...
	@Query("SELECT NEW com.cwms.entities.VendorTariff(i.cfsTariffNo, i.cfsAmndNo, i.status, i.ammendStatus) "
			...
			+ "AND i.cfsTariffNo = :cfsTariffNo AND i.cfsAmndNo = :cfsAmndNo " + "AND i.branchId = :branchId "
			+ "AND i.ammendStatus = 'N' " + "AND i.status = 'A' ")
	VendorTariff getSavedTariffValidateAmmendable(@Param("companyId") String companyId, @Param("branchId") String branchId,
			String cfsTariffNo, String cfsAmndNo);

	@Query("SELECT COUNT(e) > 0 FROM VendorTariff e " + "WHERE e.companyId = :companyId AND e.branchId = :branchId "
			+ "AND e.contractName = :contractName " + "AND (:cfsTariffNo IS NULL OR e.cfsTariffNo <> :cfsTariffNo) "
			+ "AND (:cfsAmndNo IS NULL OR :cfsAmndNo = '' OR e.cfsAmndNo <> :cfsAmndNo)	" + "AND e.status = 'A'")
	boolean existByCombosOfContractName(@Param("companyId") String companyId, @Param("branchId") String branchId,
			@Param("contractName") String contractName, String cfsTariffNo, String cfsAmndNo);
...
//	
	...
	@Query("SELECT COUNT(e) > 0 FROM VendorTariff e " + "WHERE e.companyId = :companyId AND e.branchId = :branchId "
			+ "AND e.consolerId = :consolerId " + "AND e.partyId = :partyId " + "AND e.cha = :cha "
			+ "AND e.shippingAgent = :shippingAgent " + "AND e.shippingLine = :shippingLine "
			+ "AND e.forwarderId = :forwarderId " + "AND e.importerId = :importerId "
			+ "AND (:cfsTariffNo IS NULL OR e.cfsTariffNo <> :cfsTariffNo) "
			+ "AND (:cfsAmndNo IS NULL OR :cfsAmndNo = '' OR e.cfsAmndNo <> :cfsAmndNo)	" + "AND e.status = 'A'")
	boolean existByCombosOfParties(@Param("companyId") String companyId, @Param("branchId") String branchId,
			@Param("consolerId") String consolerId, @Param("partyId") String partyId, @Param("cha") String cha,
			@Param("shippingAgent") String shippingAgent, @Param("shippingLine") String shippingLine,
			@Param("forwarderId") String forwarderId, @Param("importerId") String importerId, String cfsTariffNo,
			String cfsAmndNo);

	@Query("SELECT NEW com.cwms.entities.VendorTariff(i.cfsTariffNo, i.cfsAmndNo, i.contractName) " + "FROM VendorTariff i "
			+ "WHERE " + "i.companyId = :companyId " + "AND i.branchId = :branchId " + "AND i.status = 'A' " + "AND ("
			+ "   (:type = 'A') " + "   OR (:type = 'L' AND DATE(i.cfsValidateDate) >= CURRENT_DATE) "
			+ "   OR (:type = 'E' AND DATE(i.cfsValidateDate) < CURRENT_DATE)" + ") " + "ORDER BY i.contractName DESC")
	List<VendorTariff> getSavedTariffSearch(@Param("companyId") String companyId, @Param("branchId") String branchId,
			@Param("type") String type);

	@Query("SELECT c " + "FROM VendorTariff c " + "WHERE " + "c.companyId = :companyId "
			+ "AND c.branchId = :branchId AND c.cfsTariffNo = :cfsTariffNo AND c.cfsAmndNo = :cfsAmndNo AND status = 'A' ")
	VendorTariff getSavedTariffNotJoin(String companyId, String branchId, String cfsTariffNo, String cfsAmndNo);

	List<VendorTariff> findByCompanyIdAndBranchId(String companyId, String branchId);

	@Query(value = "SELECT * FROM cfstdtrfap WHERE cfs_tariff_no = ?1 AND company_id = ?2 AND branch_id = ?3", nativeQuery = true)
	VendorTariff findByCfsTariffNoAndCompanyIdAndBranchId(String cfsTariffNo, String companyId, String branchId);

	VendorTariff findByCompanyIdAndBranchIdAndPartyId(String companyId, String branchId, String partyId);

	@Query(value = "select c.cfsTariffNo " + "from VendorTariff c "
	...
	@Query(value = "select c.serviceId " + "from VendorTariffSrv c "
