...
public interface ImportInvoiceProformaPrintRepository extends JpaRepository<AssessmentSheet, String> {
	@Query(value = "select distinct a.container_no,DATE_FORMAT(a.approved_date,'%d %b %Y %T'),a.party_id,b.party_name,b.pan_no,ba.address_1,ba.address_2,ba.address_3,ba.pin,h.jar_dtl_desc State,"
			...
			+ "fa.pin,i.jar_dtl_desc,k.jar_dtl_desc,fa.gst_no,sl.party_name,ag.party_name, s.IGST,s.CGST,s.SGST,s.sez,a.Mail_Flag,DATE_FORMAT(a.Invoice_Date,'%d/%b/%Y'),b.FINANCE_MAIL,a.Created_By,"
			...
			+ "from cfinvsrvpro  a left outer join cfassesmentsheetpro s "
	...
	@Query(value="select a.service_id,c.service_long_desc,count(a.service_id),sum(a.execution_unit),a.service_unit,sum(a.local_amt),IFNULL(ROUND( ((sum(b.invoice_Amt)/count(a.service_id))), 3 ), 0) UNITAMT,max(a.rate),DATE_FORMAT(a.approved_date,'%d %b %Y %T'),c.range_type,sum(a.execution_unit1),"
			...
			+ "from cfinvsrvanxpro a  left outer join cfinvsrvdtlpro b on a.invoice_no=b.invoice_no and a.service_id=b.service_id   and a.company_Id=b.Company_Id and a.branch_Id=b.branch_Id  left outer join services c on a.service_id=c.service_id  "
