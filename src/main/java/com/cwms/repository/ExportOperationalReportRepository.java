...
List<Object[]> getAllAccountHolder(@Param("companyId") String companyId, 
  ...
  "cr.Grid_Location, " +
            ...
            "c.SB_No = b.SB_No AND " +
            "c.SB_Line_No = a.Line_No " +
            "LEFT OUTER JOIN party d ON a.Company_id = d.Company_id AND a.Branch_id = d.Branch_id AND " +
            "d.Party_Id = b.CHA " +
            "LEFT OUTER JOIN port p ON b.company_id = p.company_id AND " +
            "b.pod = p.port_code " +
            "LEFT OUTER JOIN party e ON b.Company_id = e.Company_id AND b.Branch_id = e.Branch_id AND " +
            "b.On_Account_Of = e.Party_Id " +
            "LEFT OUTER JOIN cfcrtg cr ON c.company_id = cr.company_id AND " +
            "c.branch_id = cr.branch_id AND " +
            "c.fin_year = cr.fin_year AND " +
...
@Query(value = "SELECT m.Party_Name, " +
        ...
        "AND b.On_Account_Of = m.Party_Id " +
        "LEFT OUTER JOIN party n ON b.Company_Id = n.Company_Id AND b.Branch_Id = n.Branch_Id " +
        "AND b.CHA = n.Party_Id " +
        "WHERE a.Company_Id = :companyId " +
        "AND a.Branch_Id = :branchId " +
        "AND a.Carting_Trans_Date BETWEEN :startDate AND :endDate " +
...
@Query(value = "SELECT DISTINCT b.Container_No, b.Container_Size, b.Container_Type, m.Party_Name AS Shipping_Line, " +
        ...
        "AND a.Container_No = b.Container_No " +
        "AND a.Gate_In_Id = b.Gate_In_Id " +
        "LEFT OUTER JOIN party m ON a.Company_Id = m.Company_Id AND a.Branch_Id = m.Branch_Id AND a.SL = m.Party_Id " +
        "LEFT OUTER JOIN party n ON a.Company_Id = n.Company_Id AND a.Branch_Id = n.Branch_Id AND a.On_Account_Of = n.Party_Id " +
        "WHERE a.Company_Id = :companyId " +
        "AND a.Branch_Id = :branchId " +
        "AND a.Container_No != '' " +
        "AND a.Process_Id = 'P00219' " +
...
@Query(value = "SELECT DISTINCT a.container_no, a.container_size, a.container_type, a.Commodity_Description, " +
        ...
        "LEFT OUTER JOIN cfexpmovementreq c ON a.company_id = c.company_id AND a.branch_id = c.branch_id " +
        "AND a.gate_out_id = c.gate_out_id AND c.Container_no = a.Container_no AND c.Profitcentre_Id = a.Profitcentre_Id " +
        "LEFT OUTER JOIN party m ON c.Company_Id = m.company_id AND c.Branch_Id = m.Branch_id AND c.On_Account_Of = m.party_id " +
        "LEFT OUTER JOIN party n ON n.company_id = c.company_id AND c.Branch_Id = n.Branch_id AND c.shipping_line = n.party_id " +
        "LEFT OUTER JOIN cfstufftally d ON d.company_id = c.company_id AND d.branch_id = c.branch_id " +
        "AND d.movement_req_id = c.movement_req_id AND c.Container_no = d.Container_no AND d.Profitcentre_Id = c.Profitcentre_Id " +
        "LEFT OUTER JOIN vessel v ON a.Company_Id = v.Company_Id AND a.Branch_Id = v.Branch_Id AND a.Vessel_Id = v.Vessel_Id " +
        "WHERE a.company_id = :companyId AND a.branch_id = :branchId " +
        "AND a.Status = 'A' "
        + "AND a.Gate_Out_Date BETWEEN :startDate AND :endDate " +
        "AND a.process_id = 'P00223' AND a.trans_type IN ('CONT', 'MOVE', 'BOWC') " +
        ...
        "AND (:sbNo IS NULL OR :sbNo = '' OR c.SB_No = :sbNo) " +
        "AND (:bookingNo IS NULL OR :bookingNo = '' OR a.delivery_order_no = :bookingNo) " +
        "AND (:exporterName IS NULL OR :exporterName = '' OR d.shipping_line = :exporterName) " +
        "AND (:accountHolderId IS NULL OR :accountHolderId = '' OR a.On_Account_Of = :accountHolderId) " +
                "AND (:cha IS NULL OR :cha = '' OR a.CHA = :cha) " +
        "ORDER BY a.Gate_Out_Date", nativeQuery = true)
List<Object[]> findEmptyGateOutContainerDetails(
...
@Query(value = "SELECT DISTINCT a.Container_No, a.Container_Size, a.Container_Type, " +
        "DATE_FORMAT(a.Gate_In_Date, '%d/%m/%Y %H:%i') AS Gate_In_Date, " +
        "DATE_FORMAT(a.De_Stuff_Date, '%d/%b/%Y %H:%i') AS Carting_Trans_Date, " +
        "IFNULL(SUM(b.Yard_Packages), '0.00') AS Yard_Packages, " +
        "IFNULL(SUM(b.actual_no_of_weight), '0.00') AS Gross_Weight, " +
        "sl.Party_Name AS Shipping_Agent, a.Container_Search_Type, " +
        "p.Party_Name AS Account_Holder, a.On_Account_Of,a.Gate_In_Id " +
        "FROM cfcrtgcn a " +
        "LEFT OUTER JOIN Party p ON a.Company_Id = p.Company_Id AND a.Branch_Id = p.Branch_Id AND a.On_Account_Of = p.Party_Id " +
        "LEFT OUTER JOIN Party sl ON a.Company_Id = sl.Company_Id AND a.Branch_Id = sl.Branch_Id AND a.shipping_agent = sl.Party_Id " +
        "LEFT OUTER JOIN cfcrtg b ON a.Company_Id = b.Company_Id AND a.Branch_Id = b.Branch_Id " +
        ...
        "AND (:sbNo IS NULL OR :sbNo = '' OR b.SB_No = :sbNo) " +
        "AND (:accountHolderId IS NULL OR :accountHolderId = '' OR a.On_Account_Of = :accountHolderId) " +
        "GROUP BY a.Gate_In_Id, a.Container_No, a.Container_Size, a.Container_Type, a.Gate_In_Date, a.De_Stuff_Date, sl.Party_Name, p.Party_Name, a.On_Account_Of, a.Container_Search_Type " +
        "ORDER BY a.Gate_In_Id", 
        nativeQuery = true)
List<Object[]> findReworkingContainerDetails(@Param("companyId") String companyId, 
                                    @Param("branchId") String branchId, 
                                    @Param("startDate") Date startDate,
...
"    AND a.On_Account_Of = p.Party_Id " +
"LEFT OUTER JOIN party p1 " +
"    ON a.company_id = p1.company_id AND a.Branch_id = p1.Branch_id " +
"    AND a.SL = p1.Party_Id " +
"WHERE a.company_id = :companyId " +
"  AND a.branch_id = :branchId " +
"  AND a.Profitcentre_Id = 'N00004' " +
...
@Query(value = "SELECT a.SB_No, " +
        ...
        "LEFT OUTER JOIN cfsbcrg c ON c.Company_Id = a.Company_Id AND c.Branch_Id = a.Branch_Id AND c.SB_Trans_Id = a.SB_Trans_Id AND c.SB_Line_No = a.SB_Line_No AND c.SB_No = a.SB_No " +
        "LEFT OUTER JOIN cfsb b ON b.company_id = c.company_id AND b.Branch_Id = c.Branch_Id AND b.SB_Trans_Id = c.SB_Trans_Id AND b.SB_No = c.SB_No " +
        "LEFT OUTER JOIN party d ON b.company_id = d.company_id AND b.Branch_id = d.Branch_id AND b.cha = d.party_id " +
        "LEFT OUTER JOIN party e ON b.company_id = e.company_id AND b.Branch_id = e.Branch_id AND b.on_account_of = e.party_id " +
        "LEFT OUTER JOIN cfEquipmentActivity cq ON a.company_id = cq.company_id AND a.Branch_id = cq.Branch_id AND a.Carting_Trans_Id = cq.De_Stuff_Id " +
        "WHERE a.company_id =:companyId AND a.branch_id = :branchId AND"
        + " a.Carting_Trans_Date BETWEEN :startDate AND :endDate " +
        "AND a.Status = 'A' "+
        "AND (:sbNo IS NULL OR :sbNo = '' OR a.SB_No = :sbNo) " +
...
@Query(value = "SELECT b.Gate_In_Type, '' as work_order_no, '' as work_order_date, " +
        ...
        "LEFT OUTER JOIN vessel vs ON st.company_id = vs.company_id AND st.Branch_id = vs.Branch_id AND st.vessel_id = vs.vessel_id " +
        "WHERE b.company_id = :companyId AND b.branch_id = :branchId AND b.status = 'A' " +
        "AND b.in_gate_in_date BETWEEN :startDate AND :endDate " +
        "AND b.Gate_In_Type IN ('Buffer', 'ONWH', 'FDS') AND b.Process_Id = 'P00234' AND b.Profitcentre_Id = 'N00004' " +
        "AND (:sbNo IS NULL OR :sbNo = '' OR d.SB_No = :sbNo) " +
        "AND (:bookingNo IS NULL OR :bookingNo = '' OR b.Delivery_Order_No = :bookingNo) " +
        "AND (:exporterName IS NULL OR :exporterName = '' OR d.exporter_id = :exporterName) " +
        "AND (:accountHolderId IS NULL OR :accountHolderId = '' OR b.On_Account_Of = :accountHolderId) " +
...
@Query(value = "SELECT DISTINCT pr.Party_Name AS accountholder, a.container_no, a.container_size, a.container_type, " +
        ...
        "AND i.Profitcentre_Id = a.Profitcentre_Id AND DATE_FORMAT(i.Period_From,'%d %b %Y') = DATE_FORMAT(a.in_gate_in_date,'%d %b %Y') " +
        "LEFT OUTER JOIN cfsb b ON a.company_id = b.company_id AND b.SB_Trans_Id = a.erp_doc_ref_no AND a.Branch_Id = b.Branch_Id " +
        "LEFT OUTER JOIN party p ON p.company_id = a.company_id AND p.branch_id = a.Branch_id AND a.sa = p.party_id " +
        "LEFT OUTER JOIN cfigm c ON c.company_id = b.company_id AND c.branch_id = b.branch_id AND a.Profitcentre_Id = c.Profitcentre_Id " +
        "AND a.VIA_NO = c.VIA_NO AND a.Doc_Ref_Date = c.Doc_Date AND a.profitcentre_id = c.profitcentre_id " +
        "LEFT OUTER JOIN party q ON q.company_id = a.company_id AND q.Branch_id = a.Branch_id AND a.on_account_of = q.party_id " +
        "LEFT OUTER JOIN party n ON a.company_id = n.company_id AND a.Branch_id = n.Branch_id AND a.sl = n.party_id " +
        "LEFT OUTER JOIN vessel v ON i.company_id = v.company_id AND i.Branch_id = v.Branch_id AND i.Vessel_Id = v.Vessel_Id " +
        "LEFT OUTER JOIN party tp ON tp.company_id = a.company_id AND tp.Branch_id = a.Branch_id AND a.Transporter = tp.party_id " +
        "LEFT OUTER JOIN party pr ON pr.Company_Id = i.Company_Id AND pr.Branch_Id = i.Branch_Id AND pr.Party_Id = i.On_Account_Of " +
        "LEFT OUTER JOIN cfequipmentactivity eq ON i.company_id = eq.company_id AND i.branch_id = eq.branch_id " +
        "AND i.stuff_id = eq.de_stuff_id " +
        "WHERE a.company_id = :companyId AND a.branch_id = :branchId AND a.Profitcentre_Id ='N00004' " +
        "AND i.MOVEMENT_TYPE ='CLP' "
...
"AND a.on_account_of = q.party_id " +
"LEFT OUTER JOIN party n " +
"ON a.company_id = n.company_id AND a.Branch_id = n.Branch_id " +
"AND a.sl = n.party_id " +
"LEFT OUTER JOIN vessel v " +
"ON i.company_id = v.company_id AND i.Branch_id = v.Branch_id " +
"AND i.Vessel_Id = v.Vessel_Id " +
"LEFT OUTER JOIN party tp " +
"ON tp.company_id = a.company_id AND tp.Branch_id = a.Branch_id " +
"AND a.Transporter = tp.party_id " +
"LEFT OUTER JOIN party pr " +
"ON pr.Company_Id = i.Company_Id AND pr.Branch_id = i.Branch_id " +
"AND pr.Party_Id = i.On_Account_Of " +
"LEFT OUTER JOIN cfequipmentactivity eq " +
"ON i.company_id = eq.company_id " +
"AND i.branch_id = eq.branch_id " +
...
//	        "  AND a.Process_Id ='P00223' " +
	    ...
	    @Query(value = 
	            ...
	            "AND b.status = 'A' " +
	            "AND a.SB_no = b.SB_no " +
	            "LEFT OUTER JOIN party w ON a.Company_Id = w.Company_Id AND a.Branch_id = w.Branch_id " +
	            "AND a.sl = w.Party_Id " +
	            "LEFT OUTER JOIN party y ON a.Company_Id = y.Company_Id AND a.Branch_id = y.Branch_id " +
	            "AND a.sa = y.Party_Id " +
	            "LEFT OUTER JOIN vessel x ON e.Company_Id = x.Company_Id AND e.Branch_id = x.Branch_id " +
	            "AND e.Vessel_id = x.Vessel_Id " +
	            "LEFT OUTER JOIN party p ON a.company_id = p.company_id AND a.Branch_id = p.Branch_id " +
	            "AND b.On_Account_Of = p.Party_Id " +
	            "LEFT OUTER JOIN party p1 ON a.company_id = p1.company_id AND a.Branch_id = p1.Branch_id " +
	            "AND a.SL = p1.Party_Id " +
	            "WHERE a.company_id = :companyId " +
	            "AND a.branch_id = :branchId " +
	            "AND a.Profitcentre_Id = 'N00004' " +
...
//	                @Param("startDate") Date startDate,
	        ...
	        @Query(value = "SELECT a.Doc_Ref_No, " +
	                ...
	                "LEFT OUTER JOIN cfsb c ON b.Company_Id = c.Company_Id AND b.Branch_Id = c.Branch_Id AND b.SB_No = c.SB_No " +
	                "AND b.SB_Trans_Id = c.SB_Trans_Id AND b.Profitcentre_Id = c.Profitcentre_Id " +
	                "LEFT OUTER JOIN party p ON c.Company_Id = p.Company_Id AND c.Branch_Id = p.Branch_Id AND c.CHA = p.Party_Id " +
	                "LEFT OUTER JOIN party q ON c.Company_Id = q.Company_Id AND c.Branch_Id = q.Branch_Id AND c.On_Account_Of = q.Party_Id " +
	                "WHERE a.Company_Id = :companyId AND a.Branch_Id = :branchId AND a.profitcentre_id = 'N00004' " +
	                "AND b.status = 'A' AND a.Process_Id = 'P00217' AND a.Status = 'A' " +
	                "AND a.Carting_Trans_Id = '' AND a.in_Gate_In_Date <= :endDate " +
	                "AND (:sbNo IS NULL OR :sbNo = '' OR c.SB_No = :sbNo) " +
	        ...
	        @Query(value = "SELECT a.Container_No, a.Container_Size, b.Party_Name, " +
	                ...
	                "AND a.Container_No = g.Container_No " +
	                "AND a.Stuff_Tally_Id = g.Stuff_Tally_Id " +
	                "LEFT OUTER JOIN party b ON b.Company_Id = a.Company_Id AND b.Branch_Id = a.Branch_Id " +
	                "AND b.Party_Id = a.Shipping_Line " +
	                "LEFT OUTER JOIN party c ON c.Company_Id = a.Company_Id AND c.Branch_Id = a.Branch_Id " +
	                "AND c.Party_Id = a.Shipping_Agent " +
	                "LEFT OUTER JOIN party d ON d.Company_Id = g.Company_Id AND d.Branch_Id = g.Branch_Id " +
	                "AND d.Party_Id = g.CHA " +
	                "LEFT OUTER JOIN Vessel e ON e.Company_Id = a.Company_Id AND e.Branch_Id = a.Branch_Id " +
	                "AND e.Vessel_Id = a.Vessel_Id " +
	                "LEFT OUTER JOIN Voyage f ON f.Company_Id = a.Company_Id AND f.Branch_Id = a.Branch_Id " +
	                "AND f.Vessel_code = a.Vessel_Id " +
	                "AND f.voyage_no = a.voyage_no " +
	                "WHERE a.Company_Id = :companyId " +
	                "AND a.Branch_Id = :branchId " +
