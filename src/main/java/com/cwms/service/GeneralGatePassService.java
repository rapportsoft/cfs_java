...
public class GeneralGatePassService {
	...
	public ResponseEntity<List<GeneralDeliveryGrid>> getDataAfterSave(String companyId, String branchId, String exBondingId, String cfBondDtlId) {
        List<GeneralDeliveryGrid> list = generalDeliveryDetailsRepo.getDataAfterSave(companyId, branchId, exBondingId, cfBondDtlId);
	...
	public List<GeneralGatePassCargo> findCfbondnocByCompanyIdAndBranchIdForCfbondGateIn(String companyId, String branchId, String partyName) {
        return generalGatePassRepository.findCfbondnocByCompanyIdAndBranchIdForCfbondGateIn(companyId, branchId, partyName);
    }
			...
			if ("add".equals(flag)) 
			{
				...
				if (gatePassDtlList != null) {
					...
					for (GeneralGatePassCargo item : gatePassDtlList) 
					{
						...
						List<String> importerNames = generalGatePassRepository.getImporterName(companyId, branchId, item.getDeliveryId(), item.getReceivingId());
						...
						if(saved!=null) 						
						{
							GeneralDeliveryDetails existing = generalDeliveryDetailsRepo.getAfterSaveForEdit(companyId, branchId, saved.getDeliveryId(),saved.getReceivingId(),
							...
							if(existing!=null)
							{
								existing.setGatePassPackages(existing.getGatePassPackages().add(saved.getGatePassPackages()));
								...
								System.out.println("Saved after upadte :"+
										generalDeliveryDetailsRepo.save(existing));
							}
...

					{
						...
						GeneralDeliveryCrg findExistingCfexbondCrg =generalDeliveryCrgRepo.findSavedGeneralDelivery(companyId, branchId, firstSavedObject.getDeliveryId(), firstSavedObject.getDepositNo(), firstSavedObject.getReceivingId());
						...
						if(findExistingCfexbondCrg!=null)
						{
							...
							System.out.println("Update row count after gate pass in delivery crg is____________"+findExistingCfexbondCrg);
						}
			...
			
			{
				...
				if (gatePassDtlList != null) 
				{
					...
					BigDecimal totalGateInPackages = gatePassDtlList.stream()
						    .map(item -> generalGatePassRepository.toUpdateGatePassCargo(
						        companyId, branchId, gatePass.getGatePassId(), item.getCommodityId(), item.getSrNo()))
...
//						System.out.println("Total Gate In Packages: " + totalGateInPackages);
					...
					for (GeneralGatePassCargo item : gatePassDtlList) 
					{
						...
						gatePassDtl.setCommodityDescription(item.getCommodityDescription());
							...
							GeneralDeliveryDetails existing = generalDeliveryDetailsRepo.getAfterSaveForEdit(companyId, branchId, item.getDeliveryId(),item.getReceivingId(),
									item.getDepositNo(),item.getCommodityId());
						
							
							
							if(existing!=null)
							{
								existing.setGatePassPackages(existing.getGatePassPackages().add(item.getGatePassPackages()).subtract(gatePassDtl.getGatePassPackages()));
								existing.setGatePassWeight(existing.getGatePassWeight().add(item.getGatePassWeight()).subtract(gatePassDtl.getGatePassWeight()));
...

					{
						...
						if(findExistingCfexbondCrg!=null)
						{
							 ...
							 findExistingCfexbondCrg.setQtyTakenOut(existingQtyTakenOut.add(totalOut).subtract(totalGateInPackages));
...
//		System.out.println("gatePassDtl_______________________" + gatePassDtlList);
	...
	public ResponseEntity<?> updateDataInExbondGridAfterBondGatePass(String companyId, String branchId, String flag, String user,
		...
		List<GeneralDeliveryGrid> savedList = new ArrayList<>();
		...
		List<GeneralDeliveryGrid> cfinbondcrgDtlList = new ArrayList<>();

		if (nocDtlObj instanceof List) {
			cfinbondcrgDtlList = objectMapper.convertValue(nocDtlObj, new TypeReference<List<GeneralDeliveryGrid>>() {
			});
		} else if (nocDtlObj instanceof Map) {
			Map<String, Object> nocDtlMap = (Map<String, Object>) nocDtlObj;
			for (Map.Entry<String, Object> entry : nocDtlMap.entrySet()) {
				GeneralDeliveryGrid cfinbondcrgDtl = objectMapper.convertValue(entry.getValue(), GeneralDeliveryGrid.class);
		...
		{
				for (GeneralDeliveryGrid grid : cfinbondcrgDtlList)
				{
					
					GeneralDeliveryGrid cf = generalDeliveryDetailsRepo.getSavedGrid(companyId, branchId,
							grid.getDeliveryId(), grid.getReceivingId(), grid.getYardLocation(),grid.getYardBlock(),grid.getBlockCellNo());

					if (cf!=null)
					{
						
						BigDecimal exitsQty =cf.getQtyTakenOut()!=null ? cf.getQtyTakenOut() :BigDecimal.ZERO;
						BigDecimal existArea =cf.getGateCellAreaReleased() !=null ? cf.getGateCellAreaReleased() : BigDecimal.ZERO;
						
						
						BigDecimal qtyOut =grid.getQtyTakenOut() !=null ? grid.getQtyTakenOut() : BigDecimal.ZERO;
						
						BigDecimal gateAreaReleased =grid.getGateCellAreaReleased() !=null ? grid.getGateCellAreaReleased() : BigDecimal.ZERO;
						
						BigDecimal exitsAreaReleased =cf.getCellAreaReleased()!=null ? cf.getCellAreaReleased() :BigDecimal.ZERO;
...
//						if (saved != null) {
							...
							if(existingCell!=null)
							{
							    ...
							    BigDecimal gridCellAreaReleased = grid.getGateCellAreaReleased() != null ? grid.getGateCellAreaReleased() : BigDecimal.ZERO;

							    if (existingCell.getCellAreaUsed().compareTo(BigDecimal.ZERO) == 0) {
							        existingCell.setCellAreaUsed(gridCellAreaReleased);  // Set to released area if it's zero or null
							    } else {
							        existingCell.setCellAreaUsed(existingCellAreaUsed.subtract(gridCellAreaReleased));  // Otherwise, subtract
							    }
...
//							    existingCell.setCellAreaUsed(existingCellAreaUsed.subtract(gridCellAreaReleased));
								...
								GeneralReceivingGrid    getExistingData = generalReceivingGateInDtlRepo.getSavedGeneralReceivingGrid(companyId, branchId,grid.getReceivingId(),grid.getYardLocation(),grid.getYardBlock(),grid.getBlockCellNo());	
							...
							if(getExistingData!=null)
							{
								...
								BigDecimal savedQtyTakenOut = grid.getQtyTakenOut() != null ? grid.getQtyTakenOut() : BigDecimal.ZERO;
								...
								BigDecimal savedAreaReleased = grid.getGateCellAreaReleased() != null ? grid.getGateCellAreaReleased() : BigDecimal.ZERO;
								...
								getExistingData.setAreaReleased(existingAreaReleased.add(savedAreaReleased));
								   ...
								   if (getExistingData.getCellAreaUsed().compareTo(BigDecimal.ZERO) == 0) {
									   getExistingData.setCellAreaUsed(savedAreaReleased);  // Set to released area if it's zero or null
								    } else {
								    	getExistingData.setCellAreaUsed(getExistingData.getCellAreaUsed().subtract(savedAreaReleased));
								    }
								   
								   
								   if (getExistingData.getCellAreaAllocated().compareTo(BigDecimal.ZERO) == 0) {
									   getExistingData.setCellAreaAllocated(savedAreaReleased);  // Set to released area if it's zero or null
								    } else {
								    	getExistingData.setCellAreaAllocated(getExistingData.getCellAreaAllocated().subtract(savedAreaReleased));
								    }
...
//								getExistingData.setCellAreaUsed(getExistingData.getCellAreaUsed().subtract(savedAreaReleased));
								   ...
								   cf.setCellAreaReleased(exitsAreaReleased.add(grid.getGateCellAreaReleased()));
				                    cf.setGateCellAreaReleased(existArea.add(gateAreaReleased));
									
//									cf.setGateCellAreaReleased(gateAreaReleased);
//									cf.setCellAreaReleased(grid.getGateCellAreaReleased());
									 saved = generalDeliveryGridRepo.save(cf);
...
//				}
	 ...
	 public BigDecimal getSumOfQtyTakenOutForCommodity(String companyId, String branchId, String exBondingId,String gatePassId, String cfBondDtlId) {
	        return generalGatePassRepository.getSumOfQtyTakenOutForCommodity(companyId, branchId, exBondingId,gatePassId, cfBondDtlId);
	    }
...
//			    	
//			        BigDecimal sumOfInbondFormGrid = generalGatePassRepository.getSumOfQtyTakenOutCommodityWise(
//			            companyId, branchId, list.getDeliveryId(),list.getReceivingId()
//			        );
//			        
//
...
//			}
		    ...
		    if (cfinbondcrg != null && cfinbondcrg.getGatePassId()!=null || !cfinbondcrg.getGatePassId().isEmpty() || !cfinbondcrg.getGatePassId().isBlank()) 	    	
		    {
		        ...
		        else
		        {
		        	 ...
		        	 List<GeneralGatePassCargo> updateAfterApprove =generalGatePassRepository.updateAfterApprove(companyId, branchId, cfinbondcrg.getGatePassId());
	 ...
	 public ResponseEntity<String> getGeneralGatePassPrint( String companyId,
			...
			List<GeneralGatePassCargo> result = generalGatePassRepository.updateAfterApprove(companyId,
					branchId, gatePassId);
			if (!result.isEmpty()) {
			...
			BigDecimal totalQtyTakenOut = result.stream()
				    .map(GeneralGatePassCargo::getGatePassPackages)           // Extract qtyTakenOut from each object
				...
				BigDecimal totalWeightOut = result.stream()
					    .map(GeneralGatePassCargo::getGatePassWeight)           // Extract qtyTakenOut from each object
					    .filter(qty -> qty != null)               // Filter out null values
					    .reduce(BigDecimal.ZERO, BigDecimal::add); // Sum the non-null values
				...
				result.stream()
				    .map(GeneralGatePassCargo::getDeliveryId)  // Extract exBondBeNo from each object
