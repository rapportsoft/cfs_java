...
import com.cwms.repository.CFSRepositaryQuatation;
import com.cwms.repository.CFSTarrifServiceRepository;
import com.cwms.repository.CFSTarrifServiceRepositoryQuatation;
...
public class Quotationservice {
	...
	@Autowired
	private CFSRepositaryQuatation tarrifRepo;

	@Autowired
	private CFSTarrifServiceRepositoryQuatation tarrifServiceRepo;

	@Autowired
	private PartyRepository partyrepo;

	@Autowired
	public SerViceRepositary serviceRepo;
	...
	@Autowired
	private CFSRepositary tarrifRepoMain;

	@Autowired
	private CFSTarrifServiceRepository tarrifServiceRepoMain;
	
	  private final ModelMapper modelMapper;

	    @Autowired
	    public Quotationservice(ModelMapper modelMapper) {
	...
	public CfsTarrif convertToCfsTarrif(CfsTarrifQuatation transferTariff) {
        return modelMapper.map(transferTariff, CfsTarrif.class);
    }

    public List<CFSTariffService> convertToCfsTariffServiceList(List<CFSTariffServiceQuatation> forTariffToTransfer) {
        return forTariffToTransfer.stream()
                .map(service -> modelMapper.map(service, CFSTariffService.class))
	...
	public byte[] getTariffReport(String companyId, String branchId, String cfsTariffNo, String cfsAmendNo, String contractName, String userId) {
	    try {
	        // Fetch services
	        List<CFSTariffServiceQuatation> forTariffReport = tarrifServiceRepo.getForTariffReport(companyId, branchId, cfsTariffNo, cfsAmendNo);

	        // Group services by serviceName
	        Map<String, List<CFSTariffServiceQuatation>> groupedServices = forTariffReport.stream()
	            .collect(Collectors.groupingBy(CFSTariffServiceQuatation::getServiceName));
...
//	        sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, 6)); // Merge cells for the title
	     ...
	     // Create the header row
	        ...
	        for (Map.Entry<String, List<CFSTariffServiceQuatation>> entry : groupedServices.entrySet()) {
	            String serviceName = entry.getKey();
	            
	            List<CFSTariffServiceQuatation> serviceList = entry.getValue();

	            // Sort the serviceList by srNo
	            serviceList.sort(Comparator.comparing(CFSTariffServiceQuatation::getSrNo));
...
//	            tariffHeadRow.createCell(0).setCellValue("Tariff Head:");
	            ...
	            // Add service details
	            for (CFSTariffServiceQuatation service : serviceList) {
...
//	                    cell6.setCellValue(service.getRate() != null ? service.getRate().toString() : "");
	...
	public byte[] getTariffReportTemplate(String companyId, String branchId, String cfsTariffNo, String cfsAmendNo, String contractName, String userId) {
	    try {
	           // Fetch services
	    	   // Create a list to hold CFSTariffServiceQuatation objects
	        List<CFSTariffServiceQuatation> forTariffReport = new ArrayList<>();

	        // Add 7 objects to the list using the constructor
	        forTariffReport.add(new CFSTariffServiceQuatation(
...
//	                    cell6.setCellValue(service.getRate() != null ? service.getRate().toString() : "");
	...
	public ResponseEntity<?> uploadTariff(String companyId, String branchId, String cfsTariffJson, MultipartFile file, String user) {
		try {
			...
			CfsTarrifQuatation cfsTariff = null;
			try {
		        cfsTariff = objectMapper.readValue(cfsTariffJson, CfsTarrifQuatation.class);
		    } catch (Exception e) {
		        return ResponseEntity.badRequest().body("Invalid CfsTariff JSON");
		    }
			
			
			CfsTarrifQuatation savedTariff = tarrifRepo.getSavedTariffNotJoin(companyId, branchId, cfsTariff.getCfsTariffNo(), cfsTariff.getCfsAmndNo());
			...
			List<CFSTariffServiceQuatation> excelFile = parseExcelFile(file, companyId, branchId, user, savedTariff.getCfsTariffNo(), formattedCfsAmmnendNo, comodityList, cargoTypeList);
			
			for(CFSTariffServiceQuatation dtl : excelFile)			
			...
			CfsTarrifQuatation ammendedTariffBefore = (CfsTarrifQuatation) savedTariff.clone();
			...
			CfsTarrifQuatation ammendedTariff = tarrifRepo.getSavedTariff(companyId, branchId, savedTariff.getCfsTariffNo(),	formattedCfsAmmnendNo);
...
//	public List<CFSTariffServiceQuatation> parseExcelFile(MultipartFile file, String companyId, String branchId, String userId, String cfsTariffNo, String cfsAmmendNo, List<Map<String, String>> commodityList, List<Map<String, String>> cargoTypeList) throws Exception {
//	    List<CFSTariffServiceQuatation> tariffServices = new ArrayList<>();
//
...
//	    	            {
//	                    CFSTariffServiceQuatation tariffService = mapToTariffService(currentServiceName, header, rowValues, serviceForTariffUpload, userId, cfsTariffNo, cfsAmmendNo, srNo, commodityList, cargoTypeList);
...
//	}
	...
	public List<CFSTariffServiceQuatation> parseExcelFile(
	        MultipartFile file,
	        String companyId,
	        String branchId,
	        String userId,
	        ...
	        while (rowIterator.hasNext()) {
	            ...
	            if (isHeader(header)) {
	                ...
	                while (rowIterator.hasNext()) {
	                    ...
	                    if (serviceForTariffUpload != null) {
	                        CFSTariffServiceQuatation tariffService = mapToTariffService(
	...
	private CFSTariffServiceQuatation mapToTariffService(String serviceName, List<String> header, List<String> rowValues, Services service, String userId, String cfsTariffNo, String cfsAmmendNo, int srNo, List<Map<String, String>> commodityList, List<Map<String, String>> cargoTypeList) {
	    CFSTariffServiceQuatation tariffService = new CFSTariffServiceQuatation();
	    ...
	    tariffService.setAmmendStatus("N");
	...
	public byte[] getAuditTrailReport(String companyId, String branchId, String cfsTariffNo, String cfsAmendNo, String contractName, String userId)
	{
		try
		{
			...
			CfsTarrifQuatation savedTariffAuditTrailReport = tarrifRepo.getSavedTariffAuditTrailReport(companyId, branchId, cfsTariffNo, cfsAmendNo);			
			List<CFSTariffServiceQuatation> forTariffAuditTrailReport = tarrifServiceRepo.getForTariffAuditTrailReport(companyId, branchId, cfsTariffNo);
		     ...
		     // Set background color to light grey
		        ...
		        Map<String, String> serviceIdToNameMap = forTariffAuditTrailReport.stream()
		        	    .collect(Collectors.toMap(
		        	        CFSTariffServiceQuatation::getServiceId, 
		        	        CFSTariffServiceQuatation::getServiceName,
		        ...
		        List<String> distinctCfsAmendNos = forTariffAuditTrailReport.stream()
		            .map(CFSTariffServiceQuatation::getCfsAmendNo)
		            .distinct()
		     ...
		     // Set column widths for clarity
		        ...
		        for (String serviceIdName : distinctServiceIdsWithNames) {
		            ...
		            for (String amendNo : distinctCfsAmendNos) {
		                ...
		                List<CFSTariffServiceQuatation> filteredRecords = forTariffAuditTrailReport.stream()
		                ...
		                CFSTariffServiceQuatation firstRecord = filteredRecords.get(0);
		                ...
		                // Add Data Rows
		                for (CFSTariffServiceQuatation record : filteredRecords) {
...
//	AMMEND TARIFF
	@Transactional
	public ResponseEntity<?> ammendTariffController(String companyId, String branchId, CfsTarrifQuatation cfsTariff,
			String user) {
		try {

			CfsTarrifQuatation savedTariff = tarrifRepo.getSavedTariffNotJoin(companyId, branchId, cfsTariff.getCfsTariffNo(),
			...
			CfsTarrifQuatation ammendedTariff = tarrifRepo.getSavedTariff(companyId, branchId, savedTariff.getCfsTariffNo(),
...
// ADD HEADER
	@Transactional
	public ResponseEntity<?> addHeaderCfsTariff(String companyId, String branchId, CfsTarrifQuatation cfsTariff, String user,
			String type) {
...
//			Checking for main Tariff Table
			
			if(type.equals("A"))
			{
				
				boolean existByCombosOfContractNameMain = tarrifRepoMain.existByCombosOfContractNameMain(companyId, branchId,
						cfsTariff.getContractName());

				if (existByCombosOfContractNameMain) {

					Map<String, Object> errorResponse = new HashMap<>();
					errorResponse.put("type", "VALIDATION_ERROR");
					errorResponse.put("field", "contractNameHeader");
					errorResponse.put("details", Map.of("contractName", cfsTariff.getContractName()));
					errorResponse.put("message", "Duplicate ContractName: " + cfsTariff.getContractName());
					return ResponseEntity.badRequest().body(errorResponse);
				}

				boolean existByCombosOfPartiesMain = tarrifRepoMain.existByCombosOfPartiesMain(companyId, branchId,
						cfsTariff.getConsolerId(), cfsTariff.getPartyId(), cfsTariff.getCha(), cfsTariff.getShippingAgent(),
						cfsTariff.getShippingLine(), cfsTariff.getForwarderId(), cfsTariff.getImporterId());

				if (existByCombosOfPartiesMain) {

					Map<String, Object> errorResponse = new HashMap<>();
					errorResponse.put("type", "VALIDATION_ERROR");
					errorResponse.put("field", "combinationHeader");
					errorResponse.put("details", Map.of("Duplicate Combination of parties", cfsTariff.getPartyId()));
					errorResponse.put("message", "Duplicate Combination of parties");
					return ResponseEntity.badRequest().body(errorResponse);
			...
			CfsTarrifQuatation savedTariff = new CfsTarrifQuatation();

			cfsTariff.setAmmendStatus(type.equals("A") ? "A" : "N");

			if (cfsTariff.getCfsTariffNo() == null || cfsTariff.getCfsTariffNo().isEmpty()) {

				String autoCFSTariffNo = processService.autoCFSTariffNo(companyId, branchId, "P00419");
				...
				CfsTarrifQuatation savedTariffNotJoin = tarrifRepo.getSavedTariffNotJoin(companyId, branchId, cfsTariff.getCfsTariffNo(), cfsTariff.getCfsAmndNo());				
				...
				if (!areTariffFieldsEqual) {
					CfsTarrifQuatation cloneTariff = (CfsTarrifQuatation) cfsTariff.clone();
			...
			if(type.equals("A"))
			{
				...
				String autoCFSTariffNo = processService.autoCFSTariffNo(companyId, branchId, "P00409");
				
//				Insert into a Main Tariff Tables...				
				CfsTarrifQuatation transferTariff = tarrifRepo.getSavedTariffToTransfer(companyId, branchId, savedTariff.getCfsTariffNo(), savedTariff.getCfsAmndNo());
				
				List<CFSTariffServiceQuatation> forTariffToTransfer = tarrifServiceRepo.getForTariffToTransfer(companyId, branchId, savedTariff.getCfsTariffNo(), savedTariff.getCfsAmndNo());
								
				CfsTarrif convertedTariff = convertToCfsTarrif(transferTariff);

				List<CFSTariffService> convertedServiceList = convertToCfsTariffServiceList(forTariffToTransfer);
				convertedTariff.setCfsAmndNo("000");
				convertedTariff.setCfsTariffDate(currentDate);
				convertedTariff.setCreatedBy(user);
				convertedTariff.setCreatedDate(currentDate);
				convertedTariff.setEditedBy(user);
				convertedTariff.setEditedDate(currentDate);
				convertedTariff.setApprovedBy(user);
				convertedTariff.setApprovedDate(currentDate);
				convertedTariff.setStatus("A");
				convertedTariff.setCfsTariffNo(autoCFSTariffNo);
				
				for(CFSTariffService  dtl : convertedServiceList)
				{
					dtl.setCfsAmendNo("000");
					dtl.setCfsTariffNo(autoCFSTariffNo);
					dtl.setCreatedBy(user);
					dtl.setCreatedDate(currentDate);
					dtl.setEditedBy(user);
					dtl.setEditedDate(currentDate);
					dtl.setApprovedBy(user);
					dtl.setApprovedDate(currentDate);
					dtl.setStatus("A");
					dtl.setAmmendStatus("A");					
				}
				...
				tarrifRepoMain.save(convertedTariff);
				tarrifServiceRepoMain.saveAll(convertedServiceList);
				
				System.out.println("updateSubmitCfsService : " + updateSubmitCfsService);
				
			}

			CfsTarrifQuatation savedTariff2 = tarrifRepo.getSavedTariff(companyId, branchId, savedTariff.getCfsTariffNo(), savedTariff.getCfsAmndNo());
	...
	public boolean areTariffFieldsEqual(CfsTarrifQuatation screenTariff, CfsTarrifQuatation dbTariff) {
	...
	public ResponseEntity<?> addDetailCfsTariffService(String companyId, String branchId,
		...
		CfsTarrifQuatation tariffToSend = new CfsTarrifQuatation();

		Map<String, Object> data = new HashMap<>();
		try {

			List<CFSTariffServiceQuatation> cfsTariffService = objectMapper.convertValue(requestData.get("cfsTariffService"),
					new TypeReference<List<CFSTariffServiceQuatation>>() {
					});

			CfsTarrifQuatation cfsTariff = objectMapper.convertValue(requestData.get("cfsTariff"), CfsTarrifQuatation.class);
			CfsTarrifQuatation getSavedTariff = tarrifRepo.getSavedTariff(companyId, branchId, cfsTariff.getCfsTariffNo(),
					cfsTariff.getCfsAmndNo());
			...
			CFSTariffServiceQuatation firstTariffService = cfsTariffService.get(0);
...
//				Ammend the Tariff
				...
				CfsTarrifQuatation newAmmendedTariff = (CfsTarrifQuatation) getSavedTariff.clone();
				...
				CfsTarrifQuatation newSavedTariff = tarrifRepo.save(newAmmendedTariff);
				tariffToSend = newSavedTariff;

				ammendTariff(companyId, branchId, getSavedTariff.getCfsTariffNo(), getSavedTariff.getCfsAmndNo(),
						formattedCfsAmmnendNo, user);

				for (CFSTariffServiceQuatation update : cfsTariffService) {
					...
					if (updateAmmendCfsService == 0) {
						CFSTariffServiceQuatation clonedDetail = (CFSTariffServiceQuatation) update.clone();
				...
				for (CFSTariffServiceQuatation addService : cfsTariffService) {
			...
			List<CFSTariffServiceQuatation> listOfSavedService = getSingleService(companyId, branchId,
			...
			if (currentSize < 7) {
				for (int i = currentSize; i < 7; i++) {
					CFSTariffServiceQuatation dummyService = cloneAndModifyService(firstTariffService, i + 1);
	...
	private CFSTariffServiceQuatation cloneAndModifyService(CFSTariffServiceQuatation firstTariffService, int srNo)
			throws CloneNotSupportedException {
		CFSTariffServiceQuatation clonedService = (CFSTariffServiceQuatation) firstTariffService.clone();
	...
	public ResponseEntity<?> getSavedTariff(String companyId, String branchId, String tarrifNo, String cfsAmndNo) {
		CfsTarrifQuatation getSavedTariffs = tarrifRepo.getSavedTariff(companyId, branchId, tarrifNo, cfsAmndNo);
	...
	public ResponseEntity<?> searchSavedTariff(String companyId, String branchId, String type) {
		...
		List<CfsTarrifQuatation> savedTariffs = tarrifRepo.getSavedTariffSearch(companyId, branchId, type);
	...
	public ResponseEntity<?> getAllServices(String companyId, String branchId) {
		...
		Map<String, Object> data = new HashMap<>();
		List<Services> allServices = serviceRepo.getActiveServicesTariff(companyId, branchId);
	...
	public List<CFSTariffServiceQuatation> getSingleService(String companyId, String branchId, String serviceId, String tariffNo,
			String cfsAmndNo) throws CloneNotSupportedException {
		List<CFSTariffServiceQuatation> updatedCfsTarrifService = new ArrayList<>();
		if (tariffNo != null) {
			List<CFSTariffServiceQuatation> savedCfsTarrifService = tarrifServiceRepo.getCFSTariffServiceByServiceId(companyId,
					branchId, tariffNo, cfsAmndNo, serviceId);
			if (savedCfsTarrifService.isEmpty()) {
				Services singleService = serviceRepo.getActiveService(companyId, branchId, serviceId);
				...
				CFSTariffServiceQuatation firstTariffService = savedCfsTarrifService.get(0);
	...
	public List<CFSTariffServiceQuatation> convertServiceToCFSTariffServiceBySaved(String companyId, String branchId,
			Services service, String tariffNo, String cfsAmndNo) {
		List<CFSTariffServiceQuatation> list = new ArrayList<>();
		BigDecimal zero = BigDecimal.ZERO;

		for (int i = 0; i < 7; i++) {
			CFSTariffServiceQuatation cfService = new CFSTariffServiceQuatation();
	...
	public void ammendTariff(String companyId, String branchId, String tariffNo, String oldAmendNo, String newAmendNo,
		...
		String updateQueryTariff = "UPDATE CfsTarrifQuatation t "
		...
		String updateQuery = "UPDATE CFSTariffServiceQuatation t "
				+ "SET t.status = 'C', t.ammendStatus = 'C', t.editedBy = :user, t.editedDate = CURRENT_TIMESTAMP  "
