...
public class Tarrifservice {
	...
	@Autowired
	private CFSRepositary tarrifRepo;

	@Autowired
	private CFSTarrifServiceRepository tarrifServiceRepo;
	...
	public byte[] getTariffReport(String companyId, String branchId, String cfsTariffNo, String cfsAmendNo, String contractName, String userId) {
	    try {
	        // Fetch services
	        List<CFSTariffService> forTariffReport = tarrifServiceRepo.getForTariffReport(companyId, branchId, cfsTariffNo, cfsAmendNo);
...
//	                    cell6.setCellValue(service.getRate() != null ? service.getRate().toString() : "");
	...
	public ResponseEntity<?> uploadTariff(String companyId, String branchId, String cfsTariffJson, MultipartFile file, String user) {
		try {
			...
			CfsTarrif cfsTariff = null;
			try {
		        cfsTariff = objectMapper.readValue(cfsTariffJson, CfsTarrif.class);
		    } catch (Exception e) {
		        return ResponseEntity.badRequest().body("Invalid CfsTariff JSON");
		    }
			
			
			CfsTarrif savedTariff = tarrifRepo.getSavedTariffNotJoin(companyId, branchId, cfsTariff.getCfsTariffNo(), cfsTariff.getCfsAmndNo());
			...
			List<CFSTariffService> excelFile = parseExcelFile(file, companyId, branchId, user, savedTariff.getCfsTariffNo(), formattedCfsAmmnendNo, comodityList, cargoTypeList);
			
			for(CFSTariffService dtl : excelFile)			
			...
			CfsTarrif ammendedTariffBefore = (CfsTarrif) savedTariff.clone();
			...
			CfsTarrif ammendedTariff = tarrifRepo.getSavedTariff(companyId, branchId, savedTariff.getCfsTariffNo(),	formattedCfsAmmnendNo);
...
//	}
	...
	public List<CFSTariffService> parseExcelFile(
	        MultipartFile file,
	        String companyId,
	        String branchId,
	        String userId,
	        ...
	        while (rowIterator.hasNext()) {
	            ...
	            if (isHeader(header)) {
	                ...
	                while (rowIterator.hasNext()) {
	                    ...
	                    if (serviceForTariffUpload != null) {
	                        CFSTariffService tariffService = mapToTariffService(
	...
	private CFSTariffService mapToTariffService(String serviceName, List<String> header, List<String> rowValues, Services service, String userId, String cfsTariffNo, String cfsAmmendNo, int srNo, List<Map<String, String>> commodityList, List<Map<String, String>> cargoTypeList) {
	    CFSTariffService tariffService = new CFSTariffService();
	...
	public byte[] getAuditTrailReport(String companyId, String branchId, String cfsTariffNo, String cfsAmendNo, String contractName, String userId)
	{
		try
		{
			...
			CfsTarrif savedTariffAuditTrailReport = tarrifRepo.getSavedTariffAuditTrailReport(companyId, branchId, cfsTariffNo, cfsAmendNo);			
			List<CFSTariffService> forTariffAuditTrailReport = tarrifServiceRepo.getForTariffAuditTrailReport(companyId, branchId, cfsTariffNo);
		     ...
		     // Set background color to light grey
		        ...
		        Map<String, String> serviceIdToNameMap = forTariffAuditTrailReport.stream()
		        	    .collect(Collectors.toMap(
		        	        CFSTariffService::getServiceId, 
		        	        CFSTariffService::getServiceName,
		        ...
		        List<String> distinctCfsAmendNos = forTariffAuditTrailReport.stream()
		            .map(CFSTariffService::getCfsAmendNo)
		            .distinct()
		     ...
		     // Set column widths for clarity
		        ...
		        for (String serviceIdName : distinctServiceIdsWithNames) {
		            ...
		            for (String amendNo : distinctCfsAmendNos) {
		                ...
		                List<CFSTariffService> filteredRecords = forTariffAuditTrailReport.stream()
		                ...
		                CFSTariffService firstRecord = filteredRecords.get(0);
		                ...
		                // Add Data Rows
		                for (CFSTariffService record : filteredRecords) {
...
//	AMMEND TARIFF
	...
	public ResponseEntity<?> ammendTariffController(String companyId, String branchId, CfsTarrif cfsTariff,
		...
		try {
			...
			CfsTarrif savedTariff = tarrifRepo.getSavedTariffNotJoin(companyId, branchId, cfsTariff.getCfsTariffNo(),
			...
			CfsTarrif ammendedTariff = tarrifRepo.getSavedTariff(companyId, branchId, savedTariff.getCfsTariffNo(),
...
// ADD HEADER
	@Transactional
	public ResponseEntity<?> addHeaderCfsTariff(String companyId, String branchId, CfsTarrif cfsTariff, String user,
			String type) {
		...
		try {
			...
			CfsTarrif savedTariff = new CfsTarrif();
			...
			if (cfsTariff.getCfsTariffNo() == null || cfsTariff.getCfsTariffNo().isEmpty()) {
				...
				CfsTarrif savedTariffNotJoin = tarrifRepo.getSavedTariffNotJoin(companyId, branchId, cfsTariff.getCfsTariffNo(), cfsTariff.getCfsAmndNo());				
				...
				if (!areTariffFieldsEqual) {
					CfsTarrif cloneTariff = (CfsTarrif) cfsTariff.clone();
			...
			CfsTarrif savedTariff2 = tarrifRepo.getSavedTariff(companyId, branchId, savedTariff.getCfsTariffNo(), savedTariff.getCfsAmndNo());
	...
	public boolean areTariffFieldsEqual(CfsTarrif screenTariff, CfsTarrif dbTariff) {
	...
	public ResponseEntity<?> addDetailCfsTariffService(String companyId, String branchId,
		...
		CfsTarrif tariffToSend = new CfsTarrif();

		Map<String, Object> data = new HashMap<>();
		try {

			List<CFSTariffService> cfsTariffService = objectMapper.convertValue(requestData.get("cfsTariffService"),
					new TypeReference<List<CFSTariffService>>() {
					});

			CfsTarrif cfsTariff = objectMapper.convertValue(requestData.get("cfsTariff"), CfsTarrif.class);
			CfsTarrif getSavedTariff = tarrifRepo.getSavedTariff(companyId, branchId, cfsTariff.getCfsTariffNo(),
					cfsTariff.getCfsAmndNo());
			...
			CFSTariffService firstTariffService = cfsTariffService.get(0);
...
//				Ammend the Tariff
				...
				CfsTarrif newAmmendedTariff = (CfsTarrif) getSavedTariff.clone();
				...
				for (CFSTariffService update : cfsTariffService) {
					...
					if (updateAmmendCfsService == 0) {
						CFSTariffService clonedDetail = (CFSTariffService) update.clone();
				...
				for (CFSTariffService addService : cfsTariffService) {
			...
			List<CFSTariffService> listOfSavedService = getSingleService(companyId, branchId,
			...
			if (currentSize < 7) {
				for (int i = currentSize; i < 7; i++) {
					CFSTariffService dummyService = cloneAndModifyService(firstTariffService, i + 1);
	...
	private CFSTariffService cloneAndModifyService(CFSTariffService firstTariffService, int srNo)
			throws CloneNotSupportedException {
		CFSTariffService clonedService = (CFSTariffService) firstTariffService.clone();
	...
	public ResponseEntity<?> getSavedTariff(String companyId, String branchId, String tarrifNo, String cfsAmndNo) {
		CfsTarrif getSavedTariffs = tarrifRepo.getSavedTariff(companyId, branchId, tarrifNo, cfsAmndNo);
	...
	public ResponseEntity<?> searchSavedTariff(String companyId, String branchId, String type) {
		...
		List<CfsTarrif> savedTariffs = tarrifRepo.getSavedTariffSearch(companyId, branchId, type);
	...
	public List<CFSTariffService> getSingleService(String companyId, String branchId, String serviceId, String tariffNo,
			String cfsAmndNo) throws CloneNotSupportedException {
		List<CFSTariffService> updatedCfsTarrifService = new ArrayList<>();
		if (tariffNo != null) {
			List<CFSTariffService> savedCfsTarrifService = tarrifServiceRepo.getCFSTariffServiceByServiceId(companyId,
			...
			if (savedCfsTarrifService.isEmpty()) {
				...
				CFSTariffService firstTariffService = savedCfsTarrifService.get(0);
	...
	public List<CFSTariffService> convertServiceToCFSTariffServiceBySaved(String companyId, String branchId,
			Services service, String tariffNo, String cfsAmndNo) {
		List<CFSTariffService> list = new ArrayList<>();
		BigDecimal zero = BigDecimal.ZERO;

		for (int i = 0; i < 7; i++) {
			CFSTariffService cfService = new CFSTariffService();
